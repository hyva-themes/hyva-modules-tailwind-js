...
import { consoleError,  consoleWarn, getJsonFile } from "./utils/index.js";
...
if (hyvaConfigTokens?.src && hyvaConfigTokens.src.startsWith("http")) {
    consoleError(
        "Design Tokens source must be a local path, external URLs are not allowed, yet!"
    );
    exit(1);
}
...
const tokensFormat =
    hyvaConfigTokens?.format || getFileType(designTokensFile) || "json";
...
const hyvaCSSvars = flattenTokens(designTokens, [], !usingFlatTokens);
const hyvaTokensCSS = `/**
